		...
		<tbody>
		 <c:forEach items="${requestScope.order_list }" var="order">
			<tr>
				...
				<td>${order.payment}</td>
				<td>${order.status}</td>
				<td><a href="removeorder?id=${order.id}">删除</a>&nbsp&nbsp&nbsp&nbsp<a href="orderItem?order_id=1">订单详情</a>&nbsp&nbsp&nbsp&nbsp<a href="listorder?id=${order.id }">评价</a></td>
			</tr>
			</c:forEach>
		</tbody>
</table>
