...
<div class="container">
	<div class="row clearfix">
		...
		<h1><font color="#FF7F50" face="fantasy">${requestScope.message}</font></h1>
			<div class="row">
			<c:forEach items="${requestScope.collect_list }" var="collect">
				<div class="col-sm-12 col-md-4">
					<div class="thumbnail">
						<img alt="300x200" class="img-responsive" src="images/${collect.image}" />
						<div class="caption">
							<h3>
								<font face="方正舒体">${collect.name }</font>
							</h3>
							¥${collect.price }
						</div>
						<div class="caption">
							<button type="button" class="btn btn-info"><a href="cart1?image=${collect.image}&name=${collect.name}&price=${collect.price}&user_id=${sessionScope.user.id}">加入购物车</a></button>
							<button type="button" class="btn btn-danger"><a href="removecollect?id=${collect.id}">删除</a></button>
						</div>
					</div>
				</div>
				</c:forEach>
